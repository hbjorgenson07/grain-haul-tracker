...
import { createTruck, updateTruck, deleteTruck, toggleTruckActive } from '@/lib/actions/admin/trucks';
import type { Truck } from '@/lib/types/database';
import { Plus, X, Check, Ban, Pencil, Trash2 } from 'lucide-react';

export function TrucksManager({ trucks }: { trucks: Truck[] }) {
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-gray-500">{trucks.length} truck(s)</p>
        <button
      ...
      {showForm && (
        <div className="mb-6 rounded-lg bg-white p-6 shadow-sm">
          <h3 className="mb-4 font-semibold text-gray-900">New Truck</h3>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-gray-50">
              ...
              <th className="px-4 py-3 text-left font-medium text-gray-600">License Plate</th>
              ...
              <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                {editingId === t.id ? (
                  ...
                  <>
                    ...
                    <td className="px-4 py-3 text-gray-500">{t.license_plate || '—'}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => { setEditingId(t.id); setShowForm(false); setError(null); }}
                        ...
                        >
                          {t.is_active ? <Ban className="h-4 w-4" /> : <Check className="h-4 w-4" />}
                        </button>
                        <button
